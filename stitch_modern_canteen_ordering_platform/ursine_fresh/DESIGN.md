...
*   **Rhythm:** A 4px baseline grid ensures vertical consistency. Spacing between card elements (e.g., photo to price) should be `md` (16px), while spacing between disparate sections should be `xl` (32px).
...
*   **Interaction Elevation:** Only when a user interacts with a card (like selecting a "Panini") does a soft, ambient shadow (0px 4px 12px rgba(0,0,0,0.05)) appear to indicate the active state.
*   **Modals:** Use a heavy backdrop blur (12px) to keep the user focused on the immediate task, such as customizing a "Wings + Chips" order.
...
*   **Product Cards:** Vertical layout on mobile with a full-width image at the top, followed by the item name (Headline-sm), a brief description (Body-sm), and the price in the Primary Green (Label-bold).
...
*   **Chips:** Used for quick category filtering (e.g., "Breakfast," "Lunch," "Drinks"). Unselected chips have a light grey fill; selected chips are solid Black with White text.
*   **Lists:** Menu lists (like the Tramazeni or Chip Roll options) use a clean 1px bottom divider. Each list item should have a chevron icon to indicate drill-down capability.
...
*   **Quantity Toggles:** A simple +/- stepper component with a primary green border, used within the cart or item customization screens.